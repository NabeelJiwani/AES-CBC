...
def listit(context):
	...
	if ans is not True:
		...
		while(True):
			...
			#If operation cancelled returns empty tuple
			if cwd == ():
				...
				print(f"{blue}Bye! See You Again.")
...
def getd_fkey(context ,cdir):
	...
	helpme = 0 #
	while True:
		key = integer(f"Enter ID of file to {context}{white}")
		if key < 0:
...
def getpass():
	...
	while limit <= 10:
		...
		pass2 = input(f"{blue}Confirm Your Password : ")
		...
		if pass1 == pass2:
			...
			limit +=0.5  								#Can be a an error so !!!!!!!!!!!!! give 20 tries in case let user learn little typing
...
def encryptfile(path, in_file, o_file, chunksize = 64*1024 ):
	...
	print("If you loose your password, then there is no possible way to recover your Data!!")
	...
	if ask :
		os.remove(os.path.join(path,o_file))
...
def main():
	...
	print(""" 
		...
		\033[1;31m\t\t\t.##.....###.\033[1;31m.#######.....
		\033[m========================================================================
		\033[1;32mCreated By \033[1;31mNJ
	...
	if choice == "1":
		...
		tries = 0
		while True:
			checksum = decryptfile(cwd, file, file+".dec")
			validatechecksum = getchecksum(cwd, file+".dec").decode("utf-8")
			if checksum == validatechecksum:
				print(f"{blue}Decryption was Sucesfull!!")
				print("Thanks for using.")
				break
			else :
				tries += 1
				print (f"{red}Decryption was not Succesfull!!")
				print ("Either password is Incorrect OR")
				print (f"The file( {file} )you provided us to Decrypt has been Tamperred or Modified.")
				print ("Try Again !!!")
				print ("Attempts: ",tries)
				os.remove(os.path.join(cwd, file+".dec"))
				time.sleep(1)
				if tries>5:
					print (f"{red}Your Attempts to Decrypt the Encrypted file limit has been reached")
					aesterisk(12)
					print (" Caution " , end="")
					aesterisk(12)
					print ("\n\nNotice :")
					print ("1. Firstly, we Don't store your password with us.")
					...
					time.sleep(5)
					decryptfile(cwd, file, file+".dec", forced = True)
